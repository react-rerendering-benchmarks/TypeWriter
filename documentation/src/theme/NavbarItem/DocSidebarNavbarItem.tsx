...
import { useActiveDocContext, useLayoutDocsSidebar, useActiveVersion } from '@docusaurus/plugin-content-docs/client';
...
export default function DocSidebarNavbarItem({
  ...
  const adjustedLabel = label === 'Extensions' && activeVersion && activeVersion <= '0.5.2' ? 'Adapters' : label;
  return <DefaultNavbarItem exact {...props} isActive={useCallback(() => activeDoc?.sidebar === sidebarId, [])} label={adjustedLabel ?? sidebarLink.label} to={sidebarLink.path} />;
}