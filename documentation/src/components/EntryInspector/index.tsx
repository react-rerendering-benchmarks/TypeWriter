...
export function EntryInspector({
  ...
  return <div className="overflow-x-hidden">
      ...
      color: blueprint.color
            ...
            <div className="text-sm hover:underline" style={{
      color: blueprint.color
    }}>
...
function FieldHeader({
  ...
  return <div className="text-gray-700 dark:text-gray-300 text-sm">
            {format(display)}
        </div>;
}
...
function ListFieldInspector({
  ...
  return <div className="transition-all duration-300 ease-in-out">
            <div className="flex space-x-1 items-center p-1 hover:bg-black hover:bg-opacity-10 dark:hover:bg-opacity-40 rounded-md cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
                <Icon icon={"mdi:chevron-right"} className="w-5 h-5 transition-transform duration-200 ease-in-out transform-gpu" style={{
        transform: `rotate(${isOpen ? 90 : 0}deg)`
            ...
            {isOpen && <div className={`overflow-hidden pl-3 space-y-3`}>
                    {value.length === 0 && <div className="text-gray-500 dark:text-gray-400 text-xs">
                            No {lastPathPart} found
                        </div>}
                    {value.map((item, index) => {
        console.log(window.globalCount++);
        return <div key={index} className="flex flex-col space-y-1">
...
function MapFieldInspector({
  ...
  return <div className="flex flex-col space-y-1">
        ...
        console.log(window.globalCount++);
        return <div key={key} className="flex flex-col space-y-1">
                        <div className="w-full text-sm">
                            <FieldInspector fieldInfo={fieldInfo.value} path={`${path}.${key}`} value={value} pages={pages} />
                        </div>
                    </div>;
      })}
            </div>
...
function EntryReferenceField({
  ...
  return <div className="flex flex-col space-y-1">
            <FieldHeader name={path} />
            <div className="text-white text-xs rounded-md p-2" style={{
...
function ItemField(props: ItemFieldProps) {
  ...
  return <div className="flex flex-col space-y-1">
            ...
            <div className="pl-2 text-xs">
                {!anyEnabled && <div className="text-gray-500 dark:text-gray-400 text-xs">
                        No item data found
                    </div>}
...
function CordPropertyField({
  value,
  display,
  color
}: CordPropertyFieldProps) {
  console.log(window.globalCount++);
...
function FloatRangeField({
  ...
  return <div className="flex flex-col space-y-1 w-full">
            ...
            <div className="flex space-x-1 w-full">
                <CordPropertyField value={start} display="Min" color="text-red-500" />
                <CordPropertyField value={end} display="Max" color="text-green-500" />
            </div>
        </div>;
}
function SkinFieldEditor({
  path,
  value
}: {
  path: string;
  ...
  return <div className="flex flex-col space-y-1">
            ...
            <div className="flex space-x-4">
                {skinUrl && <img src={skinUrl} alt="Skin Preview" className="w-24 h-auto" />}
                <div className="flex-grow space-y-2">
                    <SimpleValueField name={`${path}.texture`} value={value?.texture} icon="icon-park-solid:texture-two" />
                    <SimpleValueField name={`${path}.signature`} value={value?.signature} icon="fa6-solid:signature" />
                </div>
            </div>
        </div>;
}
