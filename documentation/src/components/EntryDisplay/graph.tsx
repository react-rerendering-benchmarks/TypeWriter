...
function getLayoutedElements(nodes: Node<EntryNodeProps, string>[], edges: Edge<any>[], options: GraphLabel): {
...
export function PageLayout({
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState(page.entries.map(entry => ({
    id: entry.id,
    type: "custom",
    position: {
      x: 0,
  ...
  return <div className="h-[35em] w-full">
            <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} nodeTypes={nodeTypes} selectionMode={SelectionMode.Partial} fitView proOptions={{
...
function EntryNode({
  ...
  return <>
            <div className="text-white rounded-md p-[0.3rem]" style={{
      backgroundColor: blueprint.color
    }}>
                <div className="flex items-center border-3 border-solid border-white dark:border-black rounded-sm" style={{
