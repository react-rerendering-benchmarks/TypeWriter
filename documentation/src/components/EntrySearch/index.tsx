...
const EntrySearch: React.FC<EntryProps> = ({
  entryName
}) => {
  ...
  if (!parsedEntry) {
    return <Admonition type="danger" title="Invalid Entry">
  ...
  const wikiCategories = ["action", "dialogue", "event", "fact", "sound", "audience", "quest", "instance", "entity", "data", "activity", "group", "static", "cinematic"];
  const category = tags.find(tag => wikiCategories.includes(tag));
  const wikiUrl = category ? `${wikiBaseUrl}/beta/adapters/${parsedEntry.adapter}Adapter/entries/${category}/${name}` : `${wikiBaseUrl}/beta/adapters/${parsedEntry.adapter}Adapter`;
  const handleOpenWiki = () => {
    window.open(wikiUrl, '_blank');
  };
  return <div className="w-full h-full flex justify-center items-center select-none pt-5 pb-10">
            <div className={`p-3 rounded-[12px] select-none shadow-md shadow-[#252525] ${colors.background}`}>
                <div className="flex items-center mb-4 space-x-2 w-[300px] p-1">
                    <Icon icon="mdi:filter" className={`text-3xl ${colors.icons} rounded-full p-1 ${colors.accent}`} />
                    <div className="flex space-x-2">
                        <button className={`flex items-center rounded-full text-white px-3 py-1 text-sm cursor-pointer ${colors.primary} ${colors.secondary}`} disabled>
                ...
                <div className={`flex items-center rounded-lg px-3 py-2 mb-4 w-96 select-none ${colors.accent}`}>
                    <Icon icon="ic:outline-search" className={`text-2xl ${colors.icons}`} />
                    <input type="text" value={`Add ${finalName}`} className={`bg-transparent border-none outline-none flex-grow ml-2 text-lg pointer-events-auto cursor-text ${colors.text}`} readOnly />
                    <Icon icon="mingcute:close-fill" className={`text-[30px] cursor-pointer rounded-full p-1 ${colors.text} ${colors.hover}`} />
                </div>
                <div className={`flex items-center rounded-lg cursor-pointer text-white w-96 mt-2 text-lg ${colors.accent} ${colors.hover}`} onClick={handleOpenWiki}>
                    <div className="flex items-center justify-center mr-2 p-4 rounded" style={{
